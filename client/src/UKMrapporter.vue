<template>
    ...
    <div class="as-container">
        <div v-if="aktivt_senter == 'arrangement'" class="rapporter-tabs container nop as-margin-top-space-4">
            ...
            <button @click="openTab('4')" :class="activeTab == '4' ? 'active' : ''" class="tab as-padding-space-1">{{ is_landsfestivalen ? 'UKM-festivalen' : 'Festivalen' }} </button>
        </div>
        
        <div v-if="aktivt_senter == 'bruker'" class="container as-container as-margin-bottom-space-4 as-margin-top-space-6">
            <h1>Rapporter tilknyttet din bruker / område</h1>
            <div class="as-margin-top-space-3">
                <PermanentNotification typeNotification="info" :tittel="'Info'" :description="'De fleste rapportene er knyttet til et arrangement, og tilgjengelig fra arrangement-systemet. Velg ditt fylke eller din kommune fra menyen til venstre, og trykk deg inn på arrangementet, og trykk deg deretter inn på det arrangementet du vil hente ut rapporter fra.'"/>
            </div>
        </div>
        <div v-if="aktivt_senter == 'network'" class="container as-container as-margin-bottom-space-4 as-margin-top-space-6">
            <h1>Nasjonale rapporter</h1>
            <div class="as-margin-top-space-3">
                <PermanentNotification typeNotification="info" :tittel="'Info'" :description="'Her finnes rapporter som gjelder alle arrangementer'"/>
            </div>
        </div>
...
const aktivt_senter = (<any>window).aktivt_senter; // bruker || network || arrangement
const is_landsfestivalen = (<any>window).is_landsfestivalen;
var tabsButtons : any = {};

// Bruker på min side velger rapporter
if(aktivt_senter == 'bruker') {
    tabsButtons = {
        [0]: [
            {title: 'Arrangementene dine', description: 'Arrangemener i kommuner eller fylker du har tilgang til', link: "?page=UKMrapporter&action=rapport&rapport=Arrangementifylke", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><path d="M17 2h-4v4.059a8.946 8.946 0 0 1 4 1.459V2zm-6 0H7v5.518a8.946 8.946 0 0 1 4-1.459V2zm1 20a7 7 0 1 0 0-14 7 7 0 0 0 0 14zm-1.225-8.519L12 11l1.225 2.481 2.738.397-1.981 1.932.468 2.727L12 17.25l-2.449 1.287.468-2.727-1.981-1.932 2.737-.397z"></path></svg>'},
            {title: 'Lokalkontakter i fylke', description: 'Kontaktinfo til alle lokalkontakter i fylke', link: "?page=UKMrapporter&action=rapport&rapport=Lokalkontakter", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><path d="M12 2h-1v9H2v1a10 10 0 0 0 17.07 7.07A10 10 0 0 0 12 2z"></path></svg>'},
        ]
    }
}
// Bruker på UKM Norge-admin velger rapporter
else if(aktivt_senter == 'network') {
    tabsButtons = {
        [0]: [
            {title: 'Antall innslag per kontaktperson', description: 'Hvor mange kontaktpersoner melder på flere innslag', link: "?page=UKMrapporter&action=rapport&rapport=Kontaktpersoner", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><path d="M17 2h-4v4.059a8.946 8.946 0 0 1 4 1.459V2zm-6 0H7v5.518a8.946 8.946 0 0 1 4-1.459V2zm1 20a7 7 0 1 0 0-14 7 7 0 0 0 0 14zm-1.225-8.519L12 11l1.225 2.481 2.738.397-1.981 1.932.468 2.727L12 17.25l-2.449 1.287.468-2.727-1.981-1.932 2.737-.397z"></path></svg>'},
            {title: 'Nøkkeltall', description: 'Hvor mange påmeldte er det i hver kategori?', link: "?page=UKMrapporter&action=rapport&rapport=NokkeltallNorge", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><path d="M12 2h-1v9H2v1a10 10 0 0 0 17.07 7.07A10 10 0 0 0 12 2z"></path></svg>'},
        ]
    }
}
// Bruker på et arrangement velger rapporter
else if(aktivt_senter == 'arrangement') {
    tabsButtons = {
        // tab 0
        [0]: [
            ...
            {title: 'Alle deltakere', description: 'Informasjon om deltakere som er påmeldt arrangementet.', link: "?page=UKMrapporter&action=rapportVue&rapportId=alleDeltakere&isLand="+ is_landsfestivalen, icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><circle cx="6" cy="4" r="2"></circle><path d="M9 7H3a1 1 0 0 0-1 1v7h2v7h4v-7h2V8a1 1 0 0 0-1-1z"></path><circle cx="17" cy="4" r="2"></circle><path d="M20.21 7.73a1 1 0 0 0-1-.73h-4.5a1 1 0 0 0-1 .73L12 14h2l-1 4h2v4h4v-4h2l-1-4h2z"></path></svg>'},
            {title: 'Skjema fra deltakere', description: 'Svar på de ekstra spørsmålene du la inn på arrangementssiden', link: "?page=UKMrapporter&action=rapportVue&rapportId=deltakerSkjema&isLand="+ is_landsfestivalen, icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><path d="M8 12.052c1.995 0 3.5-1.505 3.5-3.5s-1.505-3.5-3.5-3.5-3.5 1.505-3.5 3.5 1.505 3.5 3.5 3.5zM9 13H7c-2.757 0-5 2.243-5 5v1h12v-1c0-2.757-2.243-5-5-5zm9.364-10.364L16.95 4.05C18.271 5.373 19 7.131 19 9s-.729 3.627-2.05 4.95l1.414 1.414C20.064 13.663 21 11.403 21 9s-.936-4.663-2.636-6.364z"></path><path d="M15.535 5.464 14.121 6.88C14.688 7.445 15 8.198 15 9s-.312 1.555-.879 2.12l1.414 1.416C16.479 11.592 17 10.337 17 9s-.521-2.592-1.465-3.536z"></path></svg>'},
            {title: 'Diplomer', description: 'Last ned ferdig PDF klar for utskrift.', link: "?page=UKMrapporter&action=rapport&rapport=Diplom", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 -1 24 24" style="fill: #fff;transform: ;msFilter:;"><path d="M2 7v1l11 4 9-4V7L11 4z"></path><path d="M4 11v4.267c0 1.621 4.001 3.893 9 3.734 4-.126 6.586-1.972 7-3.467.024-.089.037-.178.037-.268V11L13 14l-5-1.667v3.213l-1-.364V12l-3-1z"></path></svg>'},
            {title: 'ID-Kort', description: 'Last ned ferdig PDF klar for utskrift.', link: "?page=UKMrapporter&action=rapport&rapport=IdKort", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 640 576"><path fill="#fff" d="M288 32L352 32C369.7 32 384 46.3 384 64L384 128L256 128L256 64C256 46.3 270.3 32 288 32zM96 96L208 96L208 128C208 154.5 229.5 176 256 176L384 176C410.5 176 432 154.5 432 128L432 96L544 96C579.3 96 608 124.7 608 160L608 480C608 515.3 579.3 544 544 544L96 544C60.7 544 32 515.3 32 480L32 160C32 124.7 60.7 96 96 96zM208 464C208 472.8 215.2 480 224 480L416 480C424.8 480 432 472.8 432 464C432 419.8 396.2 384 352 384L288 384C243.8 384 208 419.8 208 464zM320 344C350.9 344 376 318.9 376 288C376 257.1 350.9 232 320 232C289.1 232 264 257.1 264 288C264 318.9 289.1 344 320 344z"/path></svg>'},
    ...
    if(is_landsfestivalen) {
        tabsButtons[4] = [
            {title: 'Hotellbestillinger', description: 'Informasjon om hotel, rom osv.', link: "?page=UKMrapporter&action=rapportVue&rapportId=hotellbestillinger&isLand="+ is_landsfestivalen, icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: #fff;transform: ;msFilter:;"><circle cx="8" cy="11" r="3"></circle><path d="M18.205 7H12v8H4V6H2v14h2v-3h16v3h2v-4c0-.009-.005-.016-.005-.024H22V11c0-2.096-1.698-4-3.795-4z"></path></svg>'},
            ...
            {title: 'Ledere overnatting', description: 'Ledere og overnatting under festivalen.', link: "?page=UKMrapporter&action=rapportVue&rapportId=ledereOvernatting", icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="#fff" viewBox="0 0 24 24"><title>bed-clock</title><path d="M19.2 9.5L16 7.7V4H17.5V6.8L19.9 8.2L19.2 9.5M22.2 11.7C22.7 12.4 23 13.2 23 14V23H21V20H3V23H1V8H3V17H11V10.6C10.4 9.5 10 8.3 10 7C10 3.1 13.1 0 17 0S24 3.1 24 7C24 8.8 23.3 10.4 22.2 11.7M12 7C12 9.8 14.2 12 17 12S22 9.8 22 7 19.8 2 17 2 12 4.2 12 7M7 16C8.7 16 10 14.7 10 13S8.7 10 7 10 4 11.3 4 13 5.3 16 7 16Z" /></svg>'},
        ];
    }
    else {
        tabsButtons[4] = [
            {title: 'Timeplan', description: 'Informasjon om oppmøtetid.', link: "?page=UKMrapporter&action=rapport&rapport=Timeplan", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(255, 255, 255, 1);transform: ;msFilter:;"><path d="M13 8h-2v5h5v-2h-3z"></path><path d="M19.999 12c0-2.953-1.612-5.53-3.999-6.916V3a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v2.083C5.613 6.469 4.001 9.047 4.001 12a8.003 8.003 0 0 0 4.136 7H8v2.041a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V19h-.139a8 8 0 0 0 4.138-7zm-8 5.999A6.005 6.005 0 0 1 6.001 12a6.005 6.005 0 0 1 5.998-5.999c3.31 0 6 2.691 6 5.999a6.005 6.005 0 0 1-6 5.999z"></path></svg>'},
            {title: 'Overnatting', description: 'Informasjon om overnatting', link: "?page=UKMrapporter&action=rapport&rapport=OvernattingLandsbyen", icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(255, 255, 255, 1);transform: ;msFilter:;"><path d="M18.991 2H9.01C7.899 2 7 2.899 7 4.01v5.637l-4.702 4.642A1 1 0 0 0 3 16v5a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1V4.009C21 2.899 20.102 2 18.991 2zm-8.069 13.111V20H5v-5.568l2.987-2.949 2.935 3.003v.625zM13 9h-2V7h2v2zm4 8h-2v-2h2v2zm0-4h-2v-2h2v2zm0-4h-2V7h2v2z"></path><path d="M7 15h2v2H7z"></path></svg>'},
        ];
    }
}
